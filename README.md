...
| `server.js` | 本地后端服务器 | Node.js/Express 服务器，提供与 Google Apps Script 相同的 REST 接口，数据存储于本地 SQLite 数据库（`data/schedule.db`）。本地部署时使用。 |
...
| `package.json` | Node 依赖配置 | 本地部署所需的 npm 依赖声明（express、cors、better-sqlite3），运行 `npm install` 安装。 |
| `data/` | 本地数据目录 | 本地部署时自动创建，存放 SQLite 数据库文件 `schedule.db`。首次启动时若目录内存在旧版 JSON 文件，会自动迁移数据后继续使用。 |
...
| 数据存储 | Google Sheets（3个sheet） | 本地 SQLite 数据库（`data/schedule.db`） |
| 密码存储 | Google Sheets Config sheet | SQLite `config` 表（SHA-256哈希） |
| 管理员数据 | Google Sheets AdminState sheet | SQLite `admin_state` 表 |
...
- **本地模式**：在项目目录下运行以下命令，密码会重置为 `123456`：
  ```bash
  node -e "const D=require('better-sqlite3')('./data/schedule.db'); const c=require('crypto'); D.prepare(\"INSERT OR REPLACE INTO config(key,value) VALUES('admin_password_hash',?)\").run(c.createHash('sha256').update('123456').digest('hex')); console.log('密码已重置为 123456');"
  ```
- **GAS 模式**：在 Google Sheets 中找到 `Config` sheet，删除 `passwordHash` 那行，下次登录时系统会使用默认密码 `123456`

---

**Q：本地模式从旧版（JSON 文件）升级后数据还在吗？**

在。首次启动新版 `server.js` 时，程序会自动检测 `data/` 目录下的旧 JSON 文件（`admin_state.json`、`emp_accounts.json`、`schedules.json`、`submissions_*.json`、`config.json`），将数据完整迁移到 `schedule.db`，旧文件保留不删除可作为备份。迁移只在首次启动时执行一次。
...
- **本地部署**：数据在 `data/schedule.db`（SQLite 格式），可用 [DB Browser for SQLite](https://sqlitebrowser.org/) 等工具直接查看，或用脚本导出为 CSV/Excel
